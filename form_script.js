...
document.getElementById('formActuacion').addEventListener('submit', async (e) => {
   ...
   const datos = {
    ...
    if (!datos.id_usuario || !datos.id_servicio) {
        alert("Por favor, selecciona un usuario y un servicio.");
        return;
    }
   ...
   try {
       ...
       // Leer la respuesta como texto para evitar fallos de JSON
       const respuestaTexto = await res.text();
        ...
        try {
           ...
           // Si llega aquí, el PHP soltó un error de código en lugar de JSON
           console.error("Error del servidor (No es JSON):", respuestaTexto);
           alert("Error crítico en el servidor. Revisa la consola (F12).");
       }
